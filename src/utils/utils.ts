...
export const getUrlByAuthCode = (authCode: string) => {
  const filterRoute = (route) => {
    let url = '';
    if (!route || (route && !route.length)) {
      return null;
    }
    for (let i = 0; i < route.length; i += 1) {
      const item = route[i];
      // console.log('item', item);
      const { authority, path, routes } = item;
      // console.log(authority === authCode);
      if (authority === authCode) {
        url = path;
        break;
      }
      if (Array.isArray(routes)) {
        url = filterRoute(routes);
      }
    }
    return url;
  };
